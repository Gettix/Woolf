...
module.exports.run = async (bot,message,args) => {
    ...
    if(message.author.username) return send("Вы не можете забрать у себя предупреждение!");
    ...
    .setDescription("РеПредупреждение")
    .setColor('#00FF00')
    .addField("Администратор",message.author.username)
    .addField("Отнял предупреждение", `${rUser.user.username}`)
